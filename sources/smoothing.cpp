...
int smooth_t::laplacian(int i){
  ...
  for(int k=0; k < nb; ++k){
    ...
    memcpy(M+(k*3), mb, sizeof(double)*3);   
...
#ifdef DEBUG  
  printf("interpolated[%d]: (%.2f,%.2f,%.2f)\n", i, ma[0], ma[1], ma[2]); 
#endif
  ...
  do {
    ...
    for(auto t = stenc.begin(); valid && t < stenc.begin()+deg; ++t,++j){
  ...
  if(valid){
    ...
    memcpy(pa, p_ini, sizeof(double)*2);
    memcpy(ma, m_ini, sizeof(double)*3);
  }    
...
#pragma omp master
  {
    if(verbose==1)
      printf("%-18s%s","= smoothing","...");

    else if(verbose==2)
      printf("Process smoothing ... ");
