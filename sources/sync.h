...
namespace trigen {
  namespace sync {
    ...
    inline void task_reduction(int* array, std::vector<int>* heap, int* count, int stride)
    {
      int nb = heap->size();
      if(nb){
        const int offset = __sync_fetch_and_add(count,int(nb/stride));
        memcpy(array+(offset*stride), heap->data(), nb * sizeof(int));
        heap->clear();
      }        
    #pragma omp barrier
    }
    ...
    #pragma omp barrier    
      ...
      memcpy(array+off[tid], heap->data(), sizeof(int) * nb);  
      heap->clear();
    ...
    #pragma omp critical(resize)  
      {
        ...
        if(old <= chunk){
          ...
          if(verbose)
            fprintf(stderr,"warning: stenc[%d] was reallocated\n",index);
        }
      }
    }
    /* -------------------------------- */
