...
/* ------------------------------------ */
void recap(trinity::stats_t* stat){
...
/* ------------------------------------ */
void expor(trinity::stats_t* stat){
...
int main(int argc, char* argv[]) {
  ...
  auto* mesh = new trinity::mesh_t(_size,_bucket,_depth,_verb,_rounds);

  trinity::metric_t  metric(mesh,_target,_norm,_h_min,_h_max);
  trinity::partit_t  heuris(mesh->node_max(),8);
  trinity::refine_t  refine(mesh,_depth);
  trinity::swap_t    swap  (mesh);
  trinity::coarse_t  coarse(mesh,&heuris);
  trinity::smooth_t  smooth(mesh,&heuris,_depth);
  trinity::stats_t   stat[5];
