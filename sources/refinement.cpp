...
#pragma omp single
{
  ...
  memset(index, 0, (cores+1)*sizeof(int));
}        
...
#pragma omp master
  {  
    if(verbose==1)
      printf("%-18s%s","= refinement","...");

    else if(verbose==2)
      printf("Process refinement ... ");
...
#pragma omp master
  {
    ...
    *form = tools::format(span); 
    ...
    {
      printf("%10d task/sec \e[32m(%4.2f s)\e[0m [+%.1f %%]\n",
