 ...
 * can be reorderered before load and after store : all writes in other 
...
public :
  ...
  // construct
  lock_t() : _state(0) {}
  lock_t(const uint8_t value) : _state(value) {}
  lock_t(const lock_t& other) : _state(other._state.load(std::memory_order_relaxed)) {}
