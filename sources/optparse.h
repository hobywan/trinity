...
namespace optparse
{
    ...
    public:
        ...
        std::string format_option_help(unsigned int indent=2) const
        {
            ...
            for (std::list<option_t>::const_iterator it = _opts.begin(); it != _opts.end(); ++it)
            {
    ...
    private:
        ...
        const option_t &lookup_long_opt(const std::string &opt) const
        {
            ...
            for (std::map<std::string, option_t const *>::const_iterator it = _optmap_l.begin(); it != _optmap_l.end(); ++it)
            {
    ...
    namespace detail
    {
        static parser_t &add_option_group_helper(parser_t &parser,
                                                     const option_group_t &group)
        {
            for (std::list<option_t>::const_iterator oit = group._opts.begin(); oit != group._opts.end(); ++oit)
            {
                                         ...
                                         const std::vector<std::string> &v)
        {
            ...
            for (std::list<option_t>::const_iterator it = parser._opts.begin(); it != parser._opts.end(); ++it)
            {
            ...
            for (std::vector<option_group_t const *>::iterator group_it = parser._groups.begin(); group_it != parser._groups.end(); ++group_it)
            {
                ...
                for (std::list<option_t>::const_iterator it = (*group_it)->_opts.begin(); it != (*group_it)->_opts.end(); ++it)
                {
                    if (it->get_default() != "" and not parser._values.is_set(it->dest()))
                    {
                        parser._values[it->dest()] = it->get_default();
        ...
        static std::string format_help_helper(const parser_t &parser)
        {
            ...
            for (std::vector<option_group_t const *>::const_iterator it = parser._groups.begin();
