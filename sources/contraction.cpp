...
void coarse_t::identify(int i){
  ...
  for(auto v = vicin.begin(); v < vicin.end(); ++v){
    assert(i != *v);
    if(__builtin_expect(bound_src && !mesh->bound(*v),0))
      continue;
  ...
  while(skip && !bucket.empty()){
    ...
    for(auto t = stenc_beg; !skip && t < stenc_end; ++t){
      
      const int* n = mesh->get_elem(*t);
      
      // a) skip surrounding elems of (i,*it)
      if((i==n[0] && j==n[1]) || (i==n[1] && j==n[0]) ||
         (i==n[1] && j==n[2]) || (i==n[2] && j==n[1]) ||
         (i==n[2] && j==n[0]) || (i==n[0] && j==n[2])){
        // (!) check if diagonal
        skip = (++shared > 1 && bound_src && bound_dst);   // decompose into 2 steps ?
        continue;
      }
      ...
      if(!(f[0]!=f[1] && f[1]!=f[2] && f[2]!=f[0])){
        printf("problem identify: v: %d, t: %d, f:[%d,%d,%d]\n", i, *t, f[0],f[1],f[2]);
      ...
      if(mesh->counterclockwise(f)){
        ...
        skip = (l[0] > l_max || l[1] > l_max || l[2] > l_max);
        continue;
      }
      // reset
      skip = true;
...
void coarse_t::collapse(int i, int j){
  ...
  if(__builtin_expect(mesh->bound(i),0)){
    k++;
    int opp = -1;
    std::vector<int>::iterator trash = stenc.end();
    // retrieve shell elems
    for(auto t = stenc.begin(); t < stenc.end(); ++t){
      const int* n = mesh->get_elem(*t);
      // manually unrolled
      if((i==n[0] && j==n[1]) || (j==n[0] && i==n[1])) { opp=n[2]; trash=t; break; }
      if((i==n[1] && j==n[2]) || (j==n[1] && i==n[2])) { opp=n[0]; trash=t; break; }
      if((i==n[2] && j==n[0]) || (j==n[2] && i==n[0])) { opp=n[1]; trash=t; break; }
    }
#ifdef DEFERRED_UPDATES    
    ...
    // retrieve shell elems
    for(auto t = stenc.begin(); k < 2 && t < stenc.end(); ++t){
      const int* n = mesh->get_elem(*t);
      // manually unrolled
      if((i==n[0] && j==n[1]) || (j==n[0] && i==n[1])) { opp[k]=n[2]; trash[k++]=t; continue;}
      if((i==n[1] && j==n[2]) || (j==n[1] && i==n[2])) { opp[k]=n[0]; trash[k++]=t; continue;}
      if((i==n[2] && j==n[0]) || (j==n[2] && i==n[0])) { opp[k]=n[1]; trash[k++]=t; continue;}
    }
    assert(k == 2);
#ifdef DEFERRED_UPDATES    
  ...
  for(auto t = stenc.begin(); t < stenc.end() && *t > -1; ++t){
...
#pragma omp for nowait
  for(int i=0; i < nb_nodes; ++i){
    if(__builtin_expect(mesh->corner(i),0))
      ...
      activ[i] = (mesh->stenc[i].empty() ? 0:1);
  }
...
#pragma omp for
  for(int i=0; i < nb_tasks; ++i){
    ...
    int deg = mesh->vicin[k].size();
    primal[i].resize(deg+1);
    primal[i][0] = k;
    memcpy(primal[i].data()+1, mesh->vicin[k].data(), sizeof(int) * deg);
  }
}
/* ------------------------------------ */
...
#pragma omp master
  {
    if(verbose==1)
      printf("%-18s%s","= contraction","...");

    else if(verbose==2)
      printf("Process contraction ... ");
...
#pragma omp single
  {
    if(verbose==2)
    {
      printf("\n= round %2d. %*d tasks \e[0m(%2d %%)\e[0m, %*d filt. \e[0m(%2d %%)\e[0m, "
       ...
       level+1, form[0], nb_activ, (int) (nb_activ*100/nb_nodes),
                form[1], nb_tasks, (int) (nb_tasks*100/nb_activ),
                form[2], nb_indep, (int) (nb_indep*100/nb_tasks), timer::round(round));  
      fflush(stdout);
    }
  }
}
...
#pragma omp master
  {
    ...
    *form = tools::format(span); 
    ...
    {
      printf("%10d task/sec \e[32m(%4.2f s)\e[0m [%.1f %%]\e[0m\n",
      (int)std::floor(stat[1]/(end*1e-3)), (float)end/1e3, (float) (new_nodes-nb_nodes)*100/nb_elems);
    }
    else if(verbose==2)
    {
      printf("\n\n");
      printf("= nodes: %d old, %d new \e[0m(%.1f %%)\e[0m\n", nb_nodes, new_nodes, 
            (float) (new_nodes-nb_nodes)*100/nb_nodes);
      printf("= elems: %d old, %d new \e[0m(%.1f %%)\e[0m\n", nb_elems, new_elems,
            (float) (new_elems-nb_elems)*100/nb_elems);
      printf("= rate : %d merge/sec (%d tasks) \n", (int)std::floor(stat[2]/(end*1e-3)), stat[2]);
      printf("= time per step\n");
      printf("  %2d %% vicin  \e[32m(%*d ms)\e[0m\n", (int) time[0]*100/end, *form, time[0]);
      printf("  %2d %% filter \e[32m(%*d ms)\e[0m\n", (int) time[1]*100/end, *form, time[1]);
      printf("  %2d %% primal \e[32m(%*d ms)\e[0m\n", (int) time[2]*100/end, *form, time[2]);
      printf("  %2d %% indep  \e[32m(%*d ms)\e[0m\n", (int) time[3]*100/end, *form, time[3]);
